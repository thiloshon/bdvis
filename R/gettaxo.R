...
#' @export
get_taxo <- function(indf,
                     genus = FALSE,
                     verbose = FALSE,
                     progress = TRUE) {
  names(indf)[names(indf) == "Order_"] <- "Order"
  indfu <-
    sqldf("select Scientific_name from indf group by Scientific_name order by Scientific_name")
  if (file.exists("taxo.db")) {
    mytaxo <- sqldf("select * from taxo", dbname = "taxo.db")
    indfu <-
      sqldf(
        "select indfu.*, mytaxo.* from indfu left outer join mytaxo on  indfu.Scientific_name = mytaxo.Scientific_name"
      )
    indfu <- indfu[, -c(2)]
    if (progress) {
      message("Local database taxo.db is present\n")
    }
  } else {
    indfu$Kingdom[1] <- NA
    indfu$Phylum[1] <- NA
    ...
    indfu$Genus[1] <- NA
    if (progress) {
      message("Local database taxo.db is absent\n")
    }
  }
  ...
  if (dim(indfu)[1] > 0) {
    if (progress) {
      message("Processing names not present in local database ...\n")
    }
    sciname <- ""
    dat1 <- NULL
    for (i in 1:dim(indfu)[1]) {
      ...
      if (!Scientific_name == sciname) {
        dat1 <- NULL
        dat <-
          classification(get_uid(Scientific_name, verbose = verbose))
        if (!is.na(dat)) {
          dat1 <- NULL
          dat1 <-
            ldply(dat, function(x)
              x[x$rank %in% c(
                "kingdom",
                "phylum",
                "class",
      ...
      if (!is.null(dat1)) {
        names(dat1) <- dat2
        if (!is.null(dat1$kingdom)) {
          indfu$Kingdom[i] <- dat1$kingdom
        }
        if (!is.null(dat1$phylum)) {
          indfu$Phylum[i] <- dat1$phylum
        }
        if (!is.null(dat1$class)) {
          indfu$Class[i] <- dat1$class
        }
        if (!is.null(dat1$order)) {
          indfu$Order[i] <- dat1$order
        }
        if (!is.null(dat1$family)) {
          indfu$Family[i] <- dat1$family
        }
        if (!is.null(dat1$genus)) {
          indfu$Genus[i] <- dat1$genus
        }
      }
  ...
  indf <-
    sqldf("select * from indf, indfu where indf.Scientific_name = indfu.Scientific_name")
  if (file.exists("taxo.db")) {
    if (progress) {
      message("\nUpdating local database with new names\n")
    }
    sqldf(
      "insert into taxo select * from indfu where Scientific_name not in (select Scientific_name from taxo)",
      dbname = "taxo.db"
    )
  } else {
