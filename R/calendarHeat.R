...
calender_heat <- function(dates,
  ...
  dates.f <-
    data.frame(date.seq = seq(min.date, max.date, by = "days"))
  ...
  # color styles
  r2b <-
    c("#0571B0", "#92C5DE", "#F7F7F7", "#F4A582", "#CA0020") # red to blue
  r2g <-
    c("#D61818", "#FFAE63", "#FFFFBD", "#B5E384") # red to green
  w2b <-
    c("#045A8D", "#2B8CBE", "#74A9CF", "#BDC9E1", "#F1EEF6") # white to blue
    ...
    for (column in 1:ncol(panel.locs)) {
      if (panel.locs[row, column] > 0) {
        ...
        for (j in 1:12) {
          ...
          x_last_m <- dates.fsubs$woty[last.month] + 0.5
          y_last_m <- dates.fsubs$dotw[last.month] + 0.5
          grid.lines(
            x = c(x_last_m, x_last_m),
            y = c(-0.5, y_last_m),
            default.units = "native",
            gp = gpar(col = "black", lwd = 1.75)
          )
          if ((y_last_m) < 6) {
            grid.lines(
              x = c(x_last_m, x_last_m - 1),
              y = c(y_last_m, y_last_m),
              default.units = "native",
              gp = gpar(col = "black", lwd = 1.75)
            )
            grid.lines(
              x = c(x_last_m - 1, x_last_m - 1),
              y = c(y_last_m, 6.5),
              default.units = "native",
              gp = gpar(col = "black", lwd = 1.75)
            )
...
if (FALSE) {
  ...
  calender_heat(stock.data$Date, stock.data$Adj.Close, varname = "MSFT Adjusted Close")
}
