...
#' @export
bd_webmap <- function(indf) {
  bs <- indf[, c(
    which(names(indf) == "Scientific_name"),
  ...
  if (length(unique(bs$Scientific_name)) > 30 | dim(bs)[1] > 1000) {
    stop(
      "To make this map meaningful keep the number of species to
      less than 30 and number of map points to less than 1000"
    )
  }
