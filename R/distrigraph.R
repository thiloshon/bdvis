...
#' @examples
#' \dontrun{
#' distri_graph(inat, ptype = "cell", col = "tomato")
#' distri_graph(inat, ptype = "species", ylab = "Species")
#' distri_graph(inat, ptype = "efforts", col = "red")
#' distri_graph(inat, ptype = "efforts", col = "red", type = "s")
#' }
#' @export
distri_graph <- function(indf,
                         ptype = NA,
                         cumulative = F,
                         ...) {
  if (!is.na(ptype)) {
    switch(
      ...
      cell = {
        mat <-
          sqldf("select Cell_id, count(*) as Records from indf group by Cell_id")
        hist(mat$Records,
          main = "Distribution of Records per cell", xlab =
            "Records", ...
        )
      },
      species = {
        mat <-
          sqldf("select Scientific_name, count(*) as Records from indf group by Scientific_name")
        hist(mat$Records,
          main = "Distribution of Records per Species", xlab =
            "Records", ...
        )
      },
      efforts = {
        ...
        mat <-
          sqldf("select Year_, count(*) as Records from indf group by Year_ order by Year_")
        if (!cumulative) {
      ...
      effortspecies = {
        ...
        indf <- cbind(indf, Year_)
        matsp <-
          sqldf("select Scientific_name, Year_ from indf group by Scientific_name, Year_")
        mat <-
          sqldf("select Year_, count(*) as Records from matsp group by Year_ order by Year_")
        ...
        if (!cumulative) {
          ...
          for (i in 1:dim(mat1)[1]) {
            mat1$Species[i] <-
              length(unique(matsp$Scientific_name[which(matsp$Year_ < mat$Year[i])]))
          }
          plot(mat1, main = "Accumulation of species over time", ...)
        }
      },
      stop(
        "Not a valid option. See ?distrigraph for currently accepted values"
      )
    )
  } else {
