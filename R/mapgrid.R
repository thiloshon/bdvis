...
#' \code{\link{get_cell_id}} function. Currently, four types of maps can be
...
#' @examples
#' \dontrun{
#' map_grid(inat, ptype = "records", region = "India")
#' }
...
map_grid <-
  function(indf = NA,
    ...
    if (ptype == "species") {
      if (!centigrid) {
        sps <-
          sqldf("select Scientific_name, cell_id from indf group by cell_id, Scientific_name")
        cts <-
          sqldf("select cell_id, count(*) from sps group by cell_id")
      } else {
        sps <- sqldf(
          "select Scientific_name, cell_id, Centi_cell_id from indf group by cell_id, Centi_cell_id, Scientific_name"
        )
        cts <- sqldf(
          "select cell_id, Centi_cell_id, count(*) from sps group by cell_id, Centi_cell_id"
        )
      }
    }
    if (ptype == "records") {
      if (!centigrid) {
        ...
        cts <- sqldf(
          "select cell_id, Centi_cell_id, count(*) from indf group by cell_id, Centi_cell_id"
        )
      }
    }
    if (ptype == "presence") {
      if (!centigrid) {
        cts1 <-
          sqldf("select cell_id, count(*) as ct1 from indf group by cell_id")
        cts <-
          sqldf("select cell_id, 1 as ct from cts1 where ct1 <> 0")
      } else {
        cts1 <- sqldf(
          "select cell_id, Centi_cell_id, count(*) as ct1 from indf group by cell_id, Centi_cell_id"
        )
        cts <-
          sqldf("select cell_id, Centi_cell_id, 1 as ct from cts1 where ct1 <> 0")
      }
    }
    if (ptype == "complete") {
      if (!centigrid) {
    ...
    if (!is.na(bbox[1])) {
      clist <- as.data.frame(cell_id_bbox(bbox = bbox))
      cts1 <-
        sqldf("select * from cts where cell_id in (select * from clist)")
    ...
    if (!centigrid) {
      ...
      Lat <-
        -90 + (cts$Cell_id %/% 360) + (cts$Centi_cell_id %/% 10) * 0.1
      Long <-
        -180 + (cts$Cell_id %% 360) + (cts$Centi_cell_id %% 10) * 0.1
      cts <- cbind(cts, Lat, Long)
    ...
    if (ptype == "presence") {
      ...
      mybreaks <- seq(0:(ceiling(log10(max(
        cts$ct
      )))))
      myleg <- 10^mybreaks
    }
    ...
    message(paste("Rendering map...plotting ", nrow(cts), " tiles",
      sep =
        ""
    ))
    if (ptype == "presence") {
...
cell_id_bbox <- function(bbox = c(-90, 90, -180, 180)) {
  retvect <- NULL
