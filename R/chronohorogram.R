...
chronohorogram <-
  function(indf = NA,
    ...
    dat1 <-
      sqldf("select Date_collected, count(*) as ct from indf group by Date_collected")
    ...
    d <-
      as.numeric(strftime(as.Date(dat1$Date_collected, na.rm = T),
        format = "%j"
      ))
    y <-
      as.numeric(strftime(as.Date(dat1$Date_collected, na.rm = T),
    ...
    radial.plot(
      ...
      point.symbols = 20,
      grid.bg = "black",
      radial.lim = c(startyear, endyear),
      cex = ptsize,
      show.radial.grid = T,
      show.grid.labels = F,
      show.grid = T,
