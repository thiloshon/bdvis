...
taxo_tree <-
  function(indf,
    ...
    if (!is.element(sum1, names(indf))) {
      message(paste("Field", sum1, " not found in dataset \n"))
      return()
    }
    if (!is.element(sum2, names(indf))) {
      message(paste("Field", sum2, " not found in dataset \n"))
      return()
    }
    if (!is.na(title)) {
      title2 <- title
    ...
    sql1 <- paste(
      ...
      ",",
      sum2,
      ",count(*) as Recs from indf group by ",
      sum1,
      ",",
      sum2
    )
    ...
    sql2 <- paste(
      "select",
      sum1,
      ",count(*) as Gnum, sum(Recs) as Rec1 from tab1 group by ",
      sum1,
      "order by Rec1"
    )
    tab2 <- sqldf(sql2)
