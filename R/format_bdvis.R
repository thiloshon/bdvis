...
format_bdvis <-
  function(indf,
    ...
    if (!(is.null(new_fields$Latitude)) &&
      new_fields$Latitude != "Latitude") {
      if (new_fields$Latitude %in% names(indf)) {
        if ("Latitude" %in% names(indf)) {
          names(indf)[names(indf) == "Latitude"] <- "Latitude::original"
          if (!(quiet)) {
            message("Changed \"Latitude\" to \"Latitude::original\"")
          }
        }
        names(indf)[names(indf) == new_fields$Latitude] <-
          "Latitude"
        if (!(quiet)) {
          message(c("Changed \"", new_fields$Latitude, "\" to \"Latitude\""))
        }
      }
    }
    if (!(is.null(new_fields$Longitude)) &&
      new_fields$Longitude != "Longitude") {
      if (new_fields$Longitude %in% names(indf)) {
        ...
        names(indf)[names(indf) == new_fields$Longitude] <-
          "Longitude"
        if (!(quiet)) {
          message(c("Changed \"", new_fields$Longitude, "\" to \"Longitude\""))
        }
      }
    }
    if (!(is.null(new_fields$Date_collected)) &&
      ...
      if (new_fields$Date_collected %in% names(indf)) {
        if ("Date_collected" %in% names(indf)) {
          names(indf)[names(indf) == "Date_collected"] <-
            "Date_collected::original"
          if (!(quiet)) {
            message("Changed \"Date_collected\" to \"Date_collected::original\"")
          }
        }
        names(indf)[names(indf) == new_fields$Date_collected] <-
          "Date_collected"
        if (!(quiet)) {
          message(c(
            "Changed \"",
            new_fields$Date_collected,
            "\" to \"Date_collected\""
          ))
        }
      }
    ...
    if (!(is.null(new_fields$Scientific_name)) &&
      ...
      if (new_fields$Scientific_name %in% names(indf)) {
        if ("Scientific_name" %in% names(indf)) {
          names(indf)[names(indf) == "Scientific_name"] <-
            "Scientific_name::original"
          if (!(quiet)) {
            message("Changed \"Scientific_name\" to \"Scientific_name::original\"")
          }
        }
        names(indf)[names(indf) == new_fields$Scientific_name] <-
          "Scientific_name"
        if (!(quiet)) {
          message(c(
            "Changed \"",
            new_fields$Scientific_name,
            "\" to \"Scientific_name\""
          ))
        }
      }
...
bd_check_df <- function(indf) {
  if (is.na(indf) ||
    (is.data.frame(indf) &&
      nrow(indf) == 0)) {
    stop("Input data frame missing or empty")
  }
  if (!(is.data.frame(indf))) {
    stop("Provided argument is not a data.frame")
  }
  return(invisible())
}
